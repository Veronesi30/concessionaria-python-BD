...
def listar_por_string(inst_consulta):
    ...
    campos_validos = ["marca", "modelo", "cor", "proprietario"]
    campo = input("Digite o campo (marca, modelo, cor, proprietario): ").strip().lower()

    if campo not in campos_validos:
        print("Campo inválido! Escolha entre: marca, modelo, cor, proprietario.")
        return

    termo = input("Digite o texto que deseja buscar na tabela: ").strip().lower()

    # consulta parametrizada
    sql = f"SELECT * FROM carros WHERE LOWER({campo}) LIKE :termo"
    inst_consulta.execute(sql, {"termo": f"%{termo}%"})
    data = inst_consulta.fetchall()
...
def pesquisa_generica(inst_consulta):
    ...
    # Consulta SQL: pesquisa o termo em múltiplos campos de texto
    sql = f"""
        SELECT * FROM carros
    ...
    if data:
        ...
        dados_df = pd.DataFrame.from_records(
            lista_filtrada,
            columns=['Id', 'Modelo', 'Marca', 'Ano', 'Cor', 'Proprietário'],
            index='Id'
        )
        ...
        if escolha_exportar == "e":
            ...
            dados_df.to_excel(f"{nome_arquivo}.xlsx")
            print(f"Arquivo '{nome_arquivo}.xlsx' gerado com sucesso!")
 
        elif escolha_exportar == "c":
            nome_arquivo = input("Nome do arquivo: ")
            dados_df.to_csv(f"{nome_arquivo}.csv")
            print(f"Arquivo '{nome_arquivo}.csv' gerado com sucesso!")
